# Hi, I'm Ayush Shukla 👋

💻 Full Stack Developer | DevOps Enthusiast | ML Explorer

---

## 🚀 About Me

* 🎓 Computer Science Student
* 🔧 Building scalable full-stack applications
* ⚙️ Interested in DevOps, System Design, and Monitoring
* 🤖 Exploring Machine Learning (NLP & Anomaly Detection)

---

## 🛠 Tech Stack

### 💻 Languages

* C++ | Java | Python | JavaScript

### 🌐 Frontend

* React.js
* HTML | CSS

### 🔙 Backend

* Spring Boot

### 🗄 Database

* PostgreSQL

### ⚙️ DevOps & Tools

* Docker
* Jenkins
* Grafana
* Graphite
* Git & GitHub

---

## 🔥 Featured Projects

### 📌 Employee Management System (CI/CD Pipeline)

* Full-stack application using React + Spring Boot
* Integrated CI/CD pipeline using Jenkins
* Containerized using Docker
* Monitoring with Grafana & Graphite

---

### 📌 Log Anomaly Detection using BERT

* NLP-based anomaly detection on system logs
* Used Sentence Transformers
* Applied zero-shot learning approach

---

### 📌 Monitoring Stack (Grafana + Graphite)

* Built a complete monitoring system using Docker
* Visualized system metrics using Grafana dashboards

---

## 📫 Connect With Me

* LinkedIn: (https://linkedin.com/in/ayushshukla135)
* Email: ayush.shukla2023a@vitstudent.ac.in
* Portfolio: [View my site](https://ayushs135.github.io/Portfolio-Website)

---

⭐ *Always learning, building, and improving.*
